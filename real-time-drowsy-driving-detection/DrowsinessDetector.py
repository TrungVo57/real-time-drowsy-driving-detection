...
class DrowsinessDetector(QMainWindow):
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(0)  # 0 = default cam
                ...
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        ...
                        if len(points) != 0:
                            ...
                            try:
                                self.left_eye_state, self.left_conf = self.predict_eye(
                                    left_eye_roi, self.left_eye_state)
                                self.right_eye_state, self.right_conf = self.predict_eye(
                                    right_eye_roi, self.right_eye_state)
                                self.yawn_state, self.yawn_conf = self.predict_yawn(
                                    mouth_roi)
                            ...
                            if self.left_eye_state == "Close Eye" and self.right_eye_state == "Close Eye":
                                ...
                                if self.microsleeps >= 2.0:
                                    self.alert_text = "<p style='color: red; font-weight: bold;'>⚠️ Alert: Eyes closed for 2s - Drowsiness detected!</p>"
